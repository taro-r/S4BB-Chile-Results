...
# Seeding for lensing template noise
seednum = 16528  # Arbitrarily chosen seed number
...
def read_noise_map(band, yr, nlat, rlz, pbscaling=False, LTseeding=False):
    ...
    if band == 'LT':
        ...
        if LTseeding: # Seed the white noise if specified
            mseed = seed.standard_normal(size=(2,hp.nside2npix(NSIDE)))
            m = LT_NOISE * mseed
            m = m * 0.5  # Modifying noise amplitude for seeded noise
        else:
...
def read_llcdm_map(band, yr, nlat, rlz):
    ...
    if band == 'LT':
        if yr == 20:
#           nlat = 5  # Directing calcspec to grab the nlat = 5 lensing template while otherwise reading in nlat = 3 maps
...
#        m = m * .5  # MODIFYING LENSING TEMPLATE
    else:
...
def spectra_file(simtype, field, yr, nlat, rlz0, rlz1, split_bands=True, pbscaling=False):
    ...
    filename = 'spectra/noisemod/seeded/hf/phase2_spec_' 
