...
def read_llcdm_map(band, yr, nlat, rlz, comb=False, gauss=None, mult=None):
...
def read_comb_map(band, yr, nlat, rlz, pbscaling=False, gauss=None, mult=None):
    """Read CMB+fg+noise combined map, QU only"""

    if yr not in VALID_YEARS:
        raise ValueError('invalid number of years')    
    if band == 'LT':
        return read_llcdm_map(band, yr, nlat, rlz, comb=True, gauss=gauss, mult=mult)
...
def read_maps(maplist, simtype, yr, nlat, rlz, pbscaling=False, LTseeding=False, gauss=None, mult=None):
    ...
    for ml in maplist:
        if simtype == 'noise':
            ...
            m = read_comb_map(ml.name, yr, nlat, rlz, pbscaling=pbscaling, gauss=gauss, mult=mult)
        elif simtype == 'signoi':
